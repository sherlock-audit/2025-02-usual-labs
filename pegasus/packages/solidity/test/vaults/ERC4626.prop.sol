...
abstract contract ERC4626Prop is Test {
    ...
    function prop_previewDeposit(address caller, address receiver, address other, uint256 assets)
        public
    {
        vm.prank(other);
    ...
    // same transaction."
    function prop_previewMint(address caller, address receiver, address other, uint256 shares)
        public
    {
        vm.prank(other);
    ...
    // withdraw
    function prop_withdraw(address caller, address receiver, address owner, uint256 assets)
        public
    {
        ...
        assertTrue(
            caller == owner || oldAllowance != 0 || (shares == 0 && assets == 0), "access control"
        );
    }
    ...
    // s' = withdraw(a)
    // s' >= s
    function prop_RT_deposit_withdraw(address caller, uint256 assets) public virtual {
        if (!_vaultMayBeEmpty) vm.assume(IERC20(_vault_).totalSupply() > 0);
        vm.prank(caller);
    ...
    function vault_withdraw(uint256 assets, address receiver, address owner)
        internal
        returns (uint256)
    {
        return
            _call_vault(abi.encodeWithSelector(IERC4626.withdraw.selector, assets, receiver, owner));
    }

    function vault_redeem(uint256 shares, address receiver, address owner)
        internal
        returns (uint256)
    {
        return
